...
> **注意**：Helm Chart 中的镜像版本可能落后于最新构建，部署前请根据实际情况修改 `values.yaml` 中的 `image.tag`。

## 使用示例

`demo/` 目录提供了各类 DCU 资源的 Pod 示例。
...
├── cmd/main.go                          # 入口
├── internal/pkg/
│   ├── plugin/                          # Device Plugin 核心逻辑
│   ├── util/                            # DCU 工具函数与 HAMi 集成
│   └── api/                             # 设备信息 API 定义
├── deployment/
│   ├── static/                          # kubectl 部署清单
│   └── helm/                            # Helm Chart
...
This project is based in part on [HAMi](https://github.com/Project-HAMi/HAMi), licensed under the Apache License, Version 2.0.
Hygon modifications and original contributions are licensed under the Apache License, Version 2.0.

See [LICENSE](LICENSE), [NOTICE](NOTICE), and [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md) for details.
